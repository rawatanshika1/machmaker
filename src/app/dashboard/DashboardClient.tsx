  ...
  const filtered = useMemo(() => {
    return customers
      ...
      .filter((customer) => {
        ...
        if (!term) return true;
        return [customer.firstName, customer.lastName, customer.city, customer.email].some((field) => field.toLowerCase().includes(term));
      });
        ...
        <div className="mb-8 flex flex-col gap-6 rounded-[32px] border border-slate-200 bg-white p-8 shadow-card sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <label className="block text-sm font-medium text-slate-700">
              Status
              <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-brand-500">
          ...
          <label className="block text-sm font-medium text-slate-700">
            City
            <select value={cityFilter} onChange={(event) => setCityFilter(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-brand-500">
              {getCities().map((city) => (
          ...
          <label className="block text-sm font-medium text-slate-700">
            ...
            <div className="relative mt-2">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, city, email" className="w-full rounded-2xl border border-slate-300 bg-white py-3 pl-11 pr-4 text-sm text-slate-900 outline-none transition focus:border-brand-500" />
            </div>
          </label>
        </div>
        </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-slate-500">
                    No profiles match that filter. Try a different city or status.
                  </td>
                </tr>
              ) : null}
            </tbody>
